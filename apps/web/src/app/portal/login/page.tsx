...
function LoginForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const me = await import('@/lib/api').then(m => m.api.get<{ user: { role: string } }>('/api/v1/auth/me'));
    ...
    <div className="min-h-screen bg-gradient-to-br from-forest to-forest-700 flex items-center justify-center px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-4">
          ...
          {mode === 'register' && (
            <div>
              <label htmlFor="role-select" className="block text-sm font-medium text-charcoal mb-1">Role</label>
